...
cfg='./Codes/mmdetection3d/CMT/projects/configs/fusion/cmt_voxel0100_r50_800x320_cbgs.py'
...
args.config='./Codes/mmdetection3d/CMT/projects/configs/fusion/cmt_voxel0100_r50_800x320_cbgs.py'
...
tmpdir=None
performance=0
filename_tmpl='finetuned_lyft_sweep_1.pth'
...
def generate_range_image(points, H=32, W=1080, min_depth=0.0, max_depth=100.0, scan_unfolding=False):
    ...
    proj_points = np.zeros((H, W, 5 + 2), dtype=points.dtype)
    proj_points = scatter(proj_points, grid[order], points[order])

    return proj_points,saving_info

path_memory='./Codes/mmdetection3d/data/nuscenes/range_image_hi/'

print("entering the loop for validation for nuscenes:")

for i, data_p in enumerate(data_loaders[1]):

    sys.stdout.write("i \n")
    print(i)
    sys.stdout.flush()
    points=data_p['points'][0].data[0][0].detach().cpu().numpy()
    range_image,saving_info = generate_range_image(points)
    base_name=data_p['img_metas'][0].data[0][0]['pts_filename'].split('/')[-1].split('.')[0]+ '.bin'
    pickle.dump(saving_info, open(path_memory + base_name, 'wb')) #saving results


print("finished generating validation")

print("entering the loop for training for nuscenes:")

for i, data_p in enumerate(data_loaders[0]):

    sys.stdout.write("i \n")
    print(i)
    sys.stdout.flush()
    points=data_p['points'].data[0][0].detach().cpu().numpy()
    range_image,saving_info = generate_range_image(points)
    base_name=data_p['img_metas'].data[0][0]['pts_filename'].split('/')[-1].split('.')[0]+ '.bin'
    pickle.dump(saving_info, open(path_memory + base_name, 'wb')) #saving results
