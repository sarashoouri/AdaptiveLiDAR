...
from projects.mmdet3d_plugin.datasets import CustomLYFTDataset
...
def _fill_trainval_infos(lyft,
    ...
    train_lyft_infos = []
    val_lyft_infos = []
    print("length train_scenes: ")
    print(len(train_scenes))
    print(train_scenes)
    print("length val_scenes: ")
    print(len(val_scenes))
    print(val_scenes)

    for coun in range(len(lyft.scene)):
        
        cur_scene=lyft.scene[coun]
        
        #if cur_scene['name'] not in train_scenes: #check if scene is in train set
        #    continue
        ...
        cur_sample = lyft.get('sample', first_sample_token)
        print(cur_sample['scene_token'])

        while True:

            lidar_token = lyft.get('sample_data', cur_sample['data']['LIDAR_TOP'])['token']
            sd_rec = lyft.get('sample_data', cur_sample['data']['LIDAR_TOP'])
            ...
            info = {
                ...
                'scene_token' : cur_sample['scene_token'],
                'prev': cur_sample['prev'],
            ...
            if not test:
                ...
                for i in range(len(names)):
                    if names[i] in CustomLYFTDataset.NameMapping:
                        names[i] = CustomLYFTDataset.NameMapping[names[i]]
            ...
            if cur_sample['scene_token'] in train_scenes:
                train_lyft_infos.append(info)
                print('saving info for train')
            else:
                val_lyft_infos.append(info)
                print('saving info for val')

            #train_nusc_infos.append(info) #saving all the infos for train data
            
            if cur_sample['next'] == '':
                break
            else:
                cur_sample = lyft.get('sample', cur_sample['next']) #get the next data for the scene
