...
def _fill_trainval_infos(nusc,
    ...
    for cur_scene in tqdm(nusc.scene):
        ...
        while True:
            lidar_token = nusc.get('sample_data', cur_sample['data']['LIDAR_TOP'])['token']
            ...
            if not test:
                ...
                velocity = np.array(
                    [nusc.box_velocity(token)[:2] for token in cur_sample['anns']])
                valid_flag = np.array(
            ...
            if cur_sample['next'] == '':
                ...
                cur_sample = nusc.get('sample', cur_sample['next']) #get the next data for the scene
    ...
    for cur_scene in tqdm(nusc.scene):
        ...
        if cur_scene['name'] not in val_scenes: #check if the scene is in val set
            continue
        ...
        while True:
            ...
            val_nusc_infos.append(info) #saving all the infos for train data
