...
checkpoint_path='./Codes/mmdetection3d/work_dirs/checkpoints_nuscene_hi/retarining_hi_71.1.pth'
checkpoint = torch.load(checkpoint_path, map_location='cpu')  # Adjust map_location if needed

# If the checkpoint is a dictionary with a 'state_dict' key
...
tmpdir=None
performance=0
filename_tmpl='Policy_lowh_32_loww_90_four_frame_temp_init_1.5_withCVar_FFN_maskGen_final.pth'
acc_loss = 0.0
acc_loss_bbox = 0.0
acc_loss_cls = 0.0
acc_count = 0
...
if rank == 0:
    ...
    print(f"\n=== STARTING EVALUATION (rank {rank}/{world_size}) ===")
for i, data_batch in enumerate(data_loaders[1]):
    ...
    data_new = {
        ...
        'queries': data_batch['queries'],
        'pred_results': data_batch['pred_results'],
        'range_image': data_batch['range_image'],
        'points': data_batch['points'],
        'img': data_batch['img'],
    }
    with torch.no_grad():
        # turn off hard sampling, adjust temperature
        model.module.pts_bbox_head.mask_generator.mask_generator.hard = False
        model.module.pts_bbox_head.mask_generator.mask_generator.temperature = 3.0
        result, queries_new,soft_mask = model(**data_new, return_loss=False, rescale=False)
